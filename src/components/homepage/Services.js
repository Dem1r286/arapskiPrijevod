...
export default function Services() {
  ...
  const rtlClass = isArabic ? "md:flex-row-reverse" : "md:flex-row";
  ...
  return (
    ...
    >
      <div className={`flex justify-center h-[480px] items-end gap-0 lg:gap-10 w-full min-w-[350px] max-w-[1600px] px-10 md:px-5 md:px-10 flex-col ${rtlClass}`}>
    ...
    ${isArabic ? "flex-row-reverse lg:flex-col-reverse" : "flex-row lg:flex-col"}`}
        >

          <div
            dir="ltr"
            className="flex items-center overflow-hidden cursor-pointer select-none mb-3 md:mb-4 relative md:mb-40"
            ...
            >
              <AnimatePresence mode="wait">
                <motion.span
                  ...
                  className="absolute top-0 left-0 w-full font-mono font-medium"
                  style={{
                    WebkitTextStroke: bigNumberStyles.stroke,
                    color: "white",
                    fontSize: bigNumberStyles.fontSize,
          ...
          <motion.button
            variants={itemVariants}
            className="font-semibold whitespace-nowrap text-xs md:text-2xl mt-9 lg:mt-0 text-white bg-[var(--secondary)] py-2 xs:py-3 sm:py-4 md:py-6 border-3 md:border-4 border-black rounded-xl bg-transform transition-transform duration-200 hover:scale-105 cursor-pointer w-1/2 md:w-full"
            onClick={() => {
        ...
        >
          <AnimatePresence mode="wait">
            ...
            >
              <p className="font-medium text-md xs:text-lg sm:text-xl md:text-[40px] 2xl:text-3xl whitespace-nowrap">{activeService.title}</p>
              <p className="text-[10px] sm:text-xs md:text-base">{activeService.description}</p>
            </motion.div>
          </AnimatePresence>
          ...
          <motion.div variants={itemVariants} className="flex justify-center items-center flex-col w-full mt-6 md:mt-10">
            {otherServices.map((service) => (
              <div key={service.id} className="w-full">
                ...
                >
                  <p className="font-medium text-xs md:text-xl">{service.title}</p>
                  <p
                    dir="ltr"
                    className="font-medium text-2xl md:text-[30px] font-mono"
                    style={{
                      WebkitTextStroke: "1px black",
                      color: activeId === service.id ? "gray" : "white",
                    }}
                  >
